...
(function inkloopSdk() {
  ...
  /**
   * Toggles picking mode: cursor swap plus a postMessage telling the review shell the real
   * state, rather than the shell guessing from its own toggle clicks (which drifted out of sync
   * once a pick completed). Stays on across multiple picks until an explicit re-toggle.
   */
  ...
  /**
   * <body>/<html> are never a meaningful pick target - without this check, hovering empty space
   * inside them (e.g. padding below the artifact's content) still resolves event.target to one
   * of the two, pinning the highlight to that huge wrapper rect as if something were selected.
   */
