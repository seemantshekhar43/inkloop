/**
 * Static content-guidance data for `inkloop stencil`. Distinct from the review-loop mechanics
 * (open/poll/end/stop): this is authoring-time guidance for building the kind of artifact
 * inkloop reviews well, served as plain data so any caller (the CLI today) can render from one
 * source of truth rather than duplicating prose.
 *
 * Naming: "stencil" and its five sections (`fit`/`layout`/`rules`/`snags`/`loop_notes`) are
 * inkloop's own vocabulary for an inkloop-specific concern.
 */
...
/**
 * Cross-cutting visual-design floor. Every stencil's `layout`/`rules` describe *structure*; none
 * says anything about *visual* baseline quality, so an agent focused on content had no nudge
 * toward a presentable result. Reachable from every `inkloop stencil <id>` output (see
 * `runStencilCommand`), not just `loopable`, since the failure mode hits table, plan, report,
 * diagram, and comparison artifacts alike, not just mockup.
 *
 * Covers: a component-library fallback named as an option, a concrete example per component (not
 * just prose rules), a responsive-breakpoint starting pattern, a dark-mode/theme-variant prompt,
 * stencil-specific component patterns (a plan's step/timeline, a dashboard's stat/KPI tile, a
 * status-color-to-semantic worked example), and the table stencil's overflow-x safety note being
 * reachable from design_baseline's generic Tables bullet. `priority`'s CDN option below is explicit
 * about picking a palette that fits the artifact's subject, not one fixed default.
 *
 * Deliberately a documented floor, not a design-system generator: a small system-font stack, a
 * small spacing scale, a neutral palette, and basic table/card/button treatment - enough that "I
 * have no design system, what do I use" has an answer instead of silence. No runtime CSS
 * injection: any styling has to live in the artifact's own HTML, authored by the agent, or the
 * standalone-render invariant (AGENTS.md) breaks - opening the saved artifact directly must
 * render identically to opening it through inkloop. A CDN component library is named as one
 * *available* option below (per the existing Mermaid-CDN precedent in AGENTS.md), not a required
 * dependency - inkloop itself still adds nothing.
 */
...
export const DESIGN_BASELINE: DesignBaseline = {
  ...
  patterns: [
    ...
    'Timeline (dated events, as opposed to the numbered step/phase pattern above): a horizontal or vertical rail with a date label per entry rather than a sequence number, for content that\'s chronological but not a to-do sequence (a changelog, a history section in a report). Example: `<ul class="timeline">` with `.timeline { position: relative; padding-left: 28px; } .timeline::before { content: ""; position: absolute; left: 9px; top: 0; bottom: 0; width: 2px; background: var(--border); } .timeline li { position: relative; padding-bottom: 20px; } .timeline li::before { content: ""; position: absolute; left: -23px; top: 4px; width: 10px; height: 10px; border-radius: 50%; background: var(--accent); } .timeline time { display: block; font-size: 12px; color: var(--text); opacity: .7; }`.',
    "Architecture/flow diagram: available for *any* stencil, not just the `diagram` stencil - when the content being compared/reported/planned has structure, flow, or relationships that a table or prose paragraph would only describe, a Mermaid diagram often clarifies it faster (e.g. a comparison's recommended stack shown as a flowchart of how the pieces connect end to end). See the `diagram` stencil for the concrete rendering pattern (AGENTS.md's \"Rendering diagrams in an artifact\" section); this bullet exists so that guidance is reachable without first guessing to open the `diagram` stencil.",
    "Open questions/undecided items section: available for *any* stencil, not just `plan` - a dedicated section (its own heading, not interleaved with the main content) listing what's still unresolved, one item per line with a stable anchor. `plan`'s \"keep open questions in their own section\" rule is this pattern's original case; a `report`'s open questions or a `comparison`'s undecided criteria are the same shape and should reach for the same pattern rather than leaving them implicit in prose.",
    'Decision-collection row (structured reviewer input): when the artifact\'s whole point is collecting a *batch* of small decisions (triage a list, pick one of several options per row, answer a set of open questions) rather than one-off comments, give each row its own inline control (a `<select>`/toggle) instead of routing every row through inkloop\'s general element/text-range annotation loop - track changes locally, flag a changed row visually (e.g. a dashed border) so the reviewer can see what\'s pending, and fold every pending change into one consolidated note on a single "send changes" action rather than one inkloop annotation per row. This is artifact-authored JS, not a new inkloop feature: the SDK exposes `window.inkloop.addNote(comment)` as the one supported hook for an artifact\'s own script to queue a general note the same way its built-in composer does - feature-detect it (`if (window.inkloop) ...`) the same way any SDK-dependent script already has to guard the standalone-render case (see the `loopable` stencil), since it\'s undefined when the artifact is opened outside a review session. Example: `function onRowChange(id, value) { pending.set(id, value); document.getElementById(id).classList.add(\'row-changed\'); } sendBtn.addEventListener(\'click\', () => { if (!window.inkloop || pending.size === 0) return; const lines = [...pending].map(([id, v]) => `${id}: ${v}`).join(\'\\n\'); window.inkloop.addNote(\'Triage decisions:\\n\' + lines); pending.clear(); });` with `.row-changed { border: 1px dashed var(--accent); }`.',
  ],
  responsive:
...
const STENCILS: readonly Stencil[] = [
  {
    ...
    rules: [
      ...
      "Keep open questions or risks in their own section, not interleaved with the steps - see design_baseline's patterns for the reusable open-questions pattern this rule is the original case of.",
      "If steps hand off between systems or components (not just between people/time), consider a Mermaid diagram alongside the step list (see the `diagram` stencil and `design_baseline`'s patterns) - a step list shows order, not architecture.",
    ],
    snags: [
  ...
  },
  {
    ...
    rules: [
      ...
      "If the options being compared fit together as pieces of a larger system (e.g. a recommended stack), consider a Mermaid diagram showing how they connect, alongside the table (see the `diagram` stencil and `design_baseline`'s patterns) - a comparison table scores options, it doesn't show how the winner fits end to end.",
      "If some criteria are genuinely undecided rather than scored (pending more input, blocked on a dependency), list them in their own open-questions section using `design_baseline`'s pattern instead of leaving blank/tentative cells in the table to imply it.",
    ],
    snags: [
  ...
  },
  {
    ...
    rules: [
      ...
      "If the findings describe how components or systems relate (a root cause's data flow, an architecture involved in a postmortem), consider a Mermaid diagram alongside the narrative (see the `diagram` stencil and `design_baseline`'s patterns) - prose describes a flow, a diagram shows it.",
      "If open questions remain (root cause suspected but unconfirmed, a fix proposed but not yet decided), give them their own section using `design_baseline`'s open-questions pattern rather than leaving them implicit in the narrative.",
    ],
    snags: [
